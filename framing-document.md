...
- **Strict LLM Constraints & Local Question Engine:** To ensure 100% free usage and stability, the LLM acts EXCLUSIVELY as the "game master" (narrative engine). It receives the child's current progression state and returns strictly minified JSON with a short narrative wrapper (max 30 words per segment) and a single keyword for image fetching. All Math and English questions are generated locally by the backend. For English quests, the backend selects a word from a local difficulty-based list, calls the Free Dictionary API at runtime to fetch definitions or synonyms, and constructs a multiple-choice question programmatically.
- **External APIs:** Unsplash API (for dynamic background images), OpenRouter (for narrative only), and Free Dictionary API (for live English definitions/synonyms).
