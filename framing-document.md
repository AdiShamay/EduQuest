...
- **Strict LLM Constraints:** To ensure 100% free usage and avoid OpenRouter rate limits, the LLM acts purely as a "game master engine." It returns strictly minified JSON with ultra-short text (max 20-30 words per narrative segment) and a single keyword for image fetching.
